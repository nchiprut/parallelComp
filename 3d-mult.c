...
int main(int argc, char **argv) {
	...
	int period[3] = {0};
	int dims[3];
	double buf[MAT_SIZE * MAT_SIZE] = {0};
	...
	if (coords3D[1] == 0) {
		...
		MPI_Cart_rank(commCol, coords3D, &originRank);
		MPI_Ibcast(matB, MAT_SIZE*MAT_SIZE, MPI_DOUBLE, originRank, commCol, &requests[1]);
	} else {
		origin[1] = 0;
		MPI_Cart_rank(commCol, origin, &originRank);
		MPI_Ibcast(matB, MAT_SIZE*MAT_SIZE, MPI_DOUBLE, originRank, commCol, &requests[1]);
		origin[1] = coords3D[1];
	...
	*/
	...
	if (((coords3D[0] == 2)&&(coords3D[1] == 0)&&(coords3D[2] == 0)) || ((coords3D[0] == 1)&&(coords3D[1] == 0)&&(coords3D[2] == 0))){
		for (int i = 0; i<MAT_SIZE; ++i) {
			for (int j = 0; j<MAT_SIZE; ++j) {
